...
namespace CLN.API.Controllers
{
    ...
    public class ManageTRCController : CustomBaseController
    {
        ...
        private readonly IAutoGeneratedNumberRepository _autoGeneratedNumberRepository;

        public ManageTRCController(IManageTRCRepository manageTRCRepository, IManageTicketRepository manageTicketRepository, IFileManager fileManager, IAddressRepository addressRepository, IAutoGeneratedNumberRepository autoGeneratedNumberRepository)
        {
            ...
            _autoGeneratedNumberRepository = autoGeneratedNumberRepository;
        ...
        public async Task<ResponseModel> ExportTRCData(ManageTRC_Search request)
        {
            ...
            using (MemoryStream msExportDataFile = new MemoryStream())
            {
                using (ExcelPackage excelExportData = new ExcelPackage())
                {
                    ...
                    WorkSheet1.Cells[1, 11].Value = "Dispatched Date";
                    WorkSheet1.Cells[1, 12].Value = "Docket #";
                    WorkSheet1.Cells[1, 13].Value = "TRC Status";
                    WorkSheet1.Cells[1, 14].Value = "TRC Engg. Name";

                    WorkSheet1.Cells[1, 15].Value = "Created By";
                    WorkSheet1.Cells[1, 16].Value = "Created Date";
                    WorkSheet1.Cells[1, 17].Value = "Modified By";
                    WorkSheet1.Cells[1, 18].Value = "Modified Date";
                    WorkSheet1.Cells[1, 19].Value = "Status";
                    ...
                    foreach (var items in lstObj)
                    {
                        ...
                        WorkSheet1.Cells[recordIndex, 11].Value = items.PIDD_DispatchedDate;
                        WorkSheet1.Cells[recordIndex, 12].Value = items.DDB_DocketDetails;
                        WorkSheet1.Cells[recordIndex, 13].Value = items.TRCStatus;
                        WorkSheet1.Cells[recordIndex, 14].Value = items.ATE_AssignedToEngineer;
                        ...
                        WorkSheet1.Cells[recordIndex, 16].Value = items.CreatedDate;

                        WorkSheet1.Cells[recordIndex, 17].Value = items.ModifierName;
                        WorkSheet1.Cells[recordIndex, 18].Style.Numberformat.Format = DateTimeFormatInfo.CurrentInfo.ShortDatePattern;
                        WorkSheet1.Cells[recordIndex, 18].Value = items.ModifiedDate;
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GenerateQuotation(int Id)
        {
            var vQuotation = new Quotation();
            ...
            else
            {
                ...
                {
                    _response.IsSuccess = false;
                    _response.Message = "Quotation is already generated for this TRC";
                    return _response;
                }
                ...
                if (vResultObj != null)
                {
                    // Quotation Deatil Bind
                    vQuotation.Id = 0;
                    vQuotation.QuotationDate = DateTime.Now;

                    //Generate Quotation Number
                    var vAutoGenerateNumber_Request = new AutoGenerateNumber_Request()
                    {
                        Table = "tblQuotation",
                        Prefix = "QUN",
                        Length = 5,
                    ...
                    var VQuotationNumber = await _autoGeneratedNumberRepository.GetAutoGeneratedNumber(vAutoGenerateNumber_Request);

                    vQuotation.QuotationNumber = VQuotationNumber;
                    vQuotation.TRCId = vResultObj.Id;
                    vQuotation.TRCNumber = vResultObj.TRCNumber;

                    // Customer and Product Detail 
                    var vResultTicketDetailObj = await _manageTicketRepository.GetManageTicketById(Convert.ToInt32(vResultObj.TicketId));
                    if (vResultTicketDetailObj != null)
                    {
                        vQuotation.CustomerId = vResultTicketDetailObj.CD_CustomerNameId;
                        vQuotation.CustomerName = vResultTicketDetailObj.CD_CustomerName;
                        vQuotation.CustomerMobileNo = vResultTicketDetailObj.CD_CustomerMobile;
                        vQuotation.CustomerEmailId = vResultTicketDetailObj.CD_CustomerEmail;
                        vQuotation.CustomerAddressId = vResultTicketDetailObj.CD_CustomerAddressId;
                        vQuotation.CustomerAddress = vResultTicketDetailObj.CD_CustomerAddress1;
                        vQuotation.CustomerRegionId = vResultTicketDetailObj.CD_CustomerRegionId;
                        vQuotation.CustomerRegionName = vResultTicketDetailObj.CD_CustomerRegionName;
                        vQuotation.CustomerStateId = vResultTicketDetailObj.CD_CustomerStateId;
                        vQuotation.CustomerStateName = vResultTicketDetailObj.CD_CustomerStateName;
                        vQuotation.CustomerDistrictId = vResultTicketDetailObj.CD_CustomerDistrictId;
                        vQuotation.CustomerDistrictName = vResultTicketDetailObj.CD_CustomerDistrictName;
                        vQuotation.CustomerCityId = vResultTicketDetailObj.CD_CustomerCityId;
                        vQuotation.CustomerCityName = vResultTicketDetailObj.CD_CustomerCityName;
                        vQuotation.CustomerPinCode = vResultTicketDetailObj.CD_CustomerPinCode;

                        vQuotation.ProductModelId = vResultTicketDetailObj.BD_BatteryProductModelId;
                        vQuotation.ProductModel = vResultTicketDetailObj.BD_ProductModel;
                        vQuotation.ProductSerialNumberId = vResultTicketDetailObj.CD_ProductSerialNumberId;
                        vQuotation.ProductSerialNumber = vResultTicketDetailObj.CD_ProductSerialNumber;
                    }

                    vQuotation.SubTotal = 0;
                    ...
                    foreach (var item in vResultPartListObj)
                    {
                        var vQuotationPartDetails = new QuotationPartDetails();
                        ...
                        vQuotationPartDetails.UnitPrice = item.TentativeCost;
                        vQuotationPartDetails.TotalPrice = (item.Quantity * item.TentativeCost);

                        vQuotation.partDetails.Add(vQuotationPartDetails);
                    }
                }

                _response.Data = vQuotation;
            }
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> SaveQuotation(Quotation parameters)
        {
            // Save/Update
            int result = await _manageTRCRepository.SaveQuotation(parameters);
            ...
            if (result > 0)
            {
                foreach (var item in parameters.partDetails)
                {
                    var vQuotationPartDetails = new QuotationPartDetails()
                    {
                        Id = Convert.ToInt32(item.Id),
                    ...
                    int resultQuotationPartDetailsAddUpdate = await _manageTRCRepository.SaveQuotationPartDetails(vQuotationPartDetails);
                }
            }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetQuotationList(Quotation_Search parameters)
        {
            var objList = await _manageTRCRepository.GetQuotationList(parameters);
            _response.Data = objList.ToList();
            _response.Total = parameters.Total;
            return _response;
        }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetQuotationById(int Id)
        {
            if (Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                var vResultObj = await _manageTRCRepository.GetQuotationById(Id);
                if (vResultObj != null)
                {
                    var vResultPartListObj = await _manageTRCRepository.GetQuotationPartDetailsById(Id);
                    foreach (var item in vResultPartListObj)
                    {
                        var vQuotationPartDetails = new QuotationPartDetails()
                        {
                        ...
                        vResultObj.partDetails.Add(vQuotationPartDetails);
                    }
                }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> QuotationApproveNReject(Quotation_ApproveNReject parameters)
        {
            if (parameters.Id <= 0)
            {
                _response.Message = "Id is required";
            ...
            {
                int resultExpenseDetails = await _manageTRCRepository.QuotationApproveNReject(parameters);
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GenerateInvoice(int Id)
        {
            var vInvoice_Request = new Invoice_Request();
            ...
            else
            {
                ...
                var VInvoiceNumber = await _autoGeneratedNumberRepository.GetAutoGeneratedNumber(vAutoGenerateNumber_Request);

                foreach (var qItems in tblQuotation)
                {
                    vInvoice_Request.Id = 0;
                    vInvoice_Request.InvoiceDate = DateTime.Now;
                    vInvoice_Request.InvoiceNumber = VInvoiceNumber;
                    vInvoice_Request.TRCId = qItems.TRCId;
                    vInvoice_Request.SubTotal = qItems.SubTotal;
                    vInvoice_Request.TaxPerct = qItems.TaxPerct;
                    vInvoice_Request.TaxValue = qItems.TaxValue;
                    vInvoice_Request.TotalAmount = qItems.TotalAmount;

                    var vInvoicePartDetailsList = new List<InvoicePartDetails_Request>();

                    var vPartDetails = await _manageTRCRepository.GetQuotationPartDetailsById(qItems.Id);
                    foreach (var pItems in vPartDetails)
                    {
                        var vInvoicePartDetails_Request = new InvoicePartDetails_Request()
                        {
                            Id = 0,
                            InvoiceId = 0,
                            ...
                            TotalPrice = pItems.TotalPrice,
                        };

                        vInvoicePartDetailsList.Add(vInvoicePartDetails_Request);
                    }

                    vInvoice_Request.partDetails = vInvoicePartDetailsList;
                }

                // Save/Update
                int result = await _manageTRCRepository.SaveInvoice(vInvoice_Request);
                ...
                if (result > 0)
                {
                    foreach (var item in vInvoice_Request.partDetails)
                    {
                        var vQuotationPartDetails = new InvoicePartDetails_Request()
                        {
                            Id = Convert.ToInt32(item.Id),
                            InvoiceId = result,
                            SpareCategoryId = item.SpareCategoryId,
                        ...
                        int resultInvoicePartDetailsAddUpdate = await _manageTRCRepository.SaveInvoicePartDetails(vQuotationPartDetails);
                    }
                }
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> GetInvoiceList(Invoice_Search parameters)
        {
            var objList = await _manageTRCRepository.GetInvoiceList(parameters);
            _response.Data = objList.ToList();
            _response.Total = parameters.Total;
            return _response;
        }
