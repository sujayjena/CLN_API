...
namespace CLN.API.Controllers.Admin
{
    ...
    public class TicketsController : CustomBaseController
    {
        ...
        [Route("[action]")]
        [HttpPost]
        public async Task<ResponseModel> ExportTicketTypeData()
        {
            _response.IsSuccess = false;
            byte[] result;
            int recordIndex;
            ...
            IEnumerable<TicketType_Response> lstObj = await _ticketsRepository.GetTicketTypeList(parameters);
            ...
            using (MemoryStream msExportDataFile = new MemoryStream())
            {
                using (ExcelPackage excelExportData = new ExcelPackage())
                {
                    ...
                    WorkSheet1.Cells[1, 1].Value = "Ticket Warranty Status";
